...
$studentId = $_SESSION['studentid'];
    ...
    $query2 = "SELECT * FROM sfiles WHERE stuid=" . $studentId . " LIMIT $start_from, $results_per_page";
    ...
    <div class="container mt-8" style="margin-left: 1px;">
        <div class="row">
            ...
            <div class="col-md-8" style="margin-top: 10px; margin-bottom: 90px; margin-left: 50px;">
                <table class="table" style="background-color: rgba(255, 255, 255, 0);">
                    ...
                    while ($fdata = mysqli_fetch_assoc($result2)) {
                        ...
                        <tr>
                            ...
                            <?php
                             //fetch name and path
                            $query="select * from files where stuid=".$fdata["stuid"];
                ...
                <ul class="pagination">
                    <?php
                    $sql = "SELECT COUNT(*) AS total FROM sfiles WHERE stuid=" . $studentId;
                    $result = mysqli_query($conn, $sql);
                    $row = mysqli_fetch_assoc($result);
                    $total_pages = ceil($row["total"] / $results_per_page);

                    for ($i = 1; $i <= $total_pages; $i++) {
                        echo "<li class='page-item'><a class='page-link' href='subjectfiles.php?page=" . $i . "'>" . $i . "</a></li>";
                    }
                    ?>
                </ul>
            </div>
